# Keeper App
Keeper is a streamlined, responsive note-taking application built with React. It allows users to organize their thoughts by creating, viewing, editing, and deleting notes in a clean, intuitive interface.

# 🚀 Features
* Create Notes: Quickly jot down a title and content for a new note.

* View Notes: A dynamic grid layout to display all your saved thoughts.

* Edit Notes: Update existing notes on the fly to keep your information current.

* Delete Notes: Remove notes with a single click once they are no longer needed.

* Responsive Design: Fully functional on desktop, tablet, and mobile devices.

# 🛠️ Tech Stack
* Frontend: React.js

* Styling:Tailwind CSS

* State Management: React Hooks (useState, useEffect)